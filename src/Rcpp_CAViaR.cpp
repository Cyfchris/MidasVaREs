...
NumericMatrix C_GetIniParams_cav(Rcpp::NumericVector yr, Rcpp::NumericVector Xr, double q, bool As, 
  ...
  for(int i = 0; i < numInitialsRand; ++i){
    ...
    arma::colvec candidatePars = 0.9*arma::randu(numPars,1) + 0.1;
...
double objFunAL_cav(arma::colvec params, Rcpp::NumericVector yr, Rcpp::NumericVector Xr,
  ...
  double phi = 0;
  double beta0 = 0;
  double beta1 = 0; // Parameters with the lag absolute returns
  double beta2 = 0; // Autoregressive parameter
  double beta1_neg = 0; //Paramter with the lag negative returns
  double beta1_pos = 0; //Parameter with the lag positive returns
  double beta3 = 0;
  if(Uni){
    if(!As){
      beta0 = params[0]; // Intercept parameter
      beta1 = params[1]; // Parameters with the lag absolute returns
      beta2 = params[2]; // Autoregressive parameter
      phi = params[3];
      for(int i = 1; i < T; ++i){
        condVaR[i] = beta0 + beta1 * absYr[i-1] + beta2 * condVaR[i-1] ;
      }
      ...
      beta0 = params[0]; // Intercept parameter
      beta1_neg = params[1]; //Paramter with the lag negative returns
      beta1_pos = params[2]; //Parameter with the lag positive returns
      beta2 = params[3]; // Autoregressive parameter
      phi = params[4];
      for(int i = 1; i < T; ++i){
        condVaR[i] = beta0 + beta1_neg * absYr[i-1] * (yr[i-1] <= 0) + beta1_pos * absYr[i-1] * (yr[i-1] > 0)  + beta2 * condVaR[i-1];
      }
    ...
    if(!As){
      beta0 = params[0]; // Intercept parameter
      beta1 = params[1]; // Parameters with the lag absolute returns
      beta2 = params[2]; // Parameters for additional explanatory variable
      beta3 = params[3]; // Autoregressive parameter
      phi = params[4];
      for(int i = 1; i < T; ++i){
        condVaR[i] = beta0 + beta1 * absYr[i-1] + beta2 * Xr[i-1] + beta3 * condVaR[i-1];
      }
      ...
      beta1_neg = params[1]; //Paramter with the lag negative returns
      beta1_pos = params[2]; //Parameter with the lag positive returns
      beta2 = params[3]; // Parameters for the additional explanatory variable
      beta3 = params[4]; // Autoregressive parameter
      phi = params[5];
      for(int i = 1; i < T; ++i){
        condVaR[i] = beta0 + beta1_neg * absYr[i-1] * (yr[i-1] <= 0) + beta1_pos * absYr[i-1] * (yr[i-1] > 0) + beta2 * Xr[i-1] + beta3 * condVaR[i-1];
      }
  ...
  arma::colvec muAdj = mu - condES;
  double fval = 0; 
  for(int i = 0; i < T; ++i){
    double fvalLog = 0;
    double hit = q - (loss[i] <= 0);
    double dist = ((1 - q)/muAdj[i]) * exp((-loss[i]*hit)/(q*muAdj[i]));
...
NumericMatrix C_GetIniParamsAL_cav(Rcpp::NumericVector yr, Rcpp::NumericVector Xr,
  ...
  arma::colvec nInitalPhi = 5 * arma::randu(numInitialsRand) - 5;
